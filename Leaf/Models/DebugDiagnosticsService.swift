...
final class DebugDiagnosticsService {
    ...
    init(
        ...
        databaseConfig: DatabaseConfig = DebugDiagnosticsService.defaultConfig(),
        databaseEncryption: EncryptionOptions? = DebugDiagnosticsService.defaultEncryption()
    ) {
    ...
    // Own DB config/encryption helpers — mirrors the per-reader pattern
    // (InsightsReader / WorkspaceReader / DM readers each keep a private copy)
    // since those statics are `private`. Diagnostics opens the same encrypted
    // events.sqlite read-side to report row counts / WAL size.

...
/// Phase 1 (account-login) — threads the SupabaseClient actor to RootView's
/// gate so it can poll session validity. The client isn't @Observable, so we
/// pass it via a custom EnvironmentKey (same pattern as
/// `\.workspaceCascadeDeleter`).
private struct SupabaseClientForGateKey: EnvironmentKey {
    static let defaultValue: SupabaseClient? = nil
}
extension EnvironmentValues {
    var supabaseClientForGate: SupabaseClient? {
        get { self[SupabaseClientForGateKey.self] }
        set { self[SupabaseClientForGateKey.self] = newValue }
    }
}
...
struct RootView: View {
    ...
    /// Phase 1 (account-login) — hard gate. The SupabaseClient is an actor;
    /// `SupabaseOAuthService` is the @Observable login controller. Session
    /// validity is computed async into `hasValidSession` (nil = still checking
    /// → render a neutral splash rather than flashing the login form on cold
    /// launch).
    ...
    var body: some View {
        Group {
            if hasValidSession == false {
                LoginGateView(service: loginService)
            } else if hasValidSession == nil {
                // Still checking persisted session on cold launch — show a neutral
                // splash rather than flashing the login form.
                ProgressView().frame(maxWidth: .infinity, maxHeight: .infinity)
            } else if case .removedFromActiveWorkspace(let workspaceName) = workspaceReader.state {
    ...
    private func evaluateWhatsNew() {
        ...
        guard whatsNewTracker.shouldPresent(
            current: appVersion,

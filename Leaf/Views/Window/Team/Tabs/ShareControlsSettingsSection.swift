...
struct ShareControlsSettingsSection: View {
  ...
  /// the toggles render the prior workspace's `[ShareSource: Bool]` map
  /// and writes target the now-active workspace via `activeStore` —
  /// silent cross-workspace data corruption (per the parallel-agent audit
  /// punch list, HIGH severity). Mirrors the ActiveTokens / Pending
  /// requests `.task(id:)` fix from earlier this branch.
  ...
  var body: some View {
    LeafSection(
      ...
      description:
        "Per-source toggles. Off by default for everything except commits, Linear issues, and detected decisions/blockers. Teammates only see what you explicitly turn on."
    ) {
...
private struct NeverSharedItem {
  ...
  static let all: [NeverSharedItem] = [
    .init(
      title: "Secrets-bearing files", description: ".env*, .git/config, .aws/credentials, .ssh/"),
    .init(title: "Large files", description: "Anything over 100 MB"),
    .init(
      title: "AI prompts and responses",
      description: "Conversation content with Claude, Cursor, and other AI assistants"),
    .init(
      title: "Personal apps",
      description: "Signal, Messages, Discord, Spotify — not in default deny-list but always opt-in"
    ),
  ]
}
...
extension ShareSource {
  ...
  var explainer: String {
    ...
    case .detectedBlockers:
      return "Patterns flagged as blockers (stuck Linear tickets, “blocked on” messages)."

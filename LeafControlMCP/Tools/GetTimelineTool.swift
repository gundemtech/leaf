...
struct GetTimelineTool: ToolExecutor {
    ...
    static let definition: ToolDefinition = {
        ...
        return ToolDefinition(
            name: "get_timeline",
            description: "Return top applications by active time for the given period. Data comes from the local LeafControl agent — raw metadata never leaves the device.",
        ...
        guard FileManager.default.fileExists(atPath: dbURL.path) else {
            return ToolCallResult(
                content: [.text(TextContent(
                    text: "LeafControl database not found at \(dbURL.path). Enable 'Background collection' in Settings first."
                ))],
                isError: true
            )
        }

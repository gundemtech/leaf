...
final class DatabaseIntegrationTests: XCTestCase {
    ...
    override func setUp() async throws {
        tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("leafcontrol-integration-\(UUID().uuidString)", isDirectory: true)
    ...
    func testReaderCannotWrite() throws {
        ...
        XCTAssertThrowsError(try reader.write(RawEvent(signalType: .attention, bundleID: "X"))) { error in
            guard let lcError = error as? LeafControlError else {
                return XCTFail("Expected LeafControlError, got \(error)")
            }
...
extension LeafControlError: Equatable {
    public static func == (lhs: LeafControlError, rhs: LeafControlError) -> Bool {
        switch (lhs, rhs) {

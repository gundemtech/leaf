...
final class SupabaseClientEnsureFreshSessionTests: XCTestCase {
  ...
  private func makeJWT(pubkey: String, userID: String = "00000000-0000-0000-0000-000000000aaa")
    -> String
  {
  ...
  /// takes the persisted-refresh path (single /auth/v1/token call) — the no-anon
  /// equivalent of the old signup→register→token bootstrap. Tests' counters that
  /// assert exactly one bootstrap /auth/v1/token call still hold.
  private func seededStore(userID: String = "00000000-0000-0000-0000-000000000aaa")
    -> SupabaseSessionStore
  {
    let dir = FileManager.default.temporaryDirectory
      .appendingPathComponent("leaf-fresh-\(UUID().uuidString)", isDirectory: true)
  ...
  func testEnsureFreshSession_DuringBootstrap_WaitsForBootstrapNotUnauthorized() async throws {
    ...
    // refresh-token so ensureAuthenticated() enters .bootstrapping via the
    // gated /auth/v1/token (persisted-refresh) call — the no-anon equivalent
    // of the old gated signup→token chain.
    ...
    try? seedStore.write(
      PersistedSession(
        ...
        userID: "00000000-0000-0000-0000-000000000aaa", savedAtMs: 1))
    let client = SupabaseClient(
      baseURL: baseURL, anonKey: anonKey,
      urlSession: makeSession(),
      identity: fixedIdentity(),
    ...
    // Phase 1 (account-login) — anonymous bootstrap removed; bootstrap is now a
    // single persisted-refresh /auth/v1/token call (no signup). The fresh
    ...
    XCTAssertEqual(
      counter.get("/auth/v1/signup"), 0, "no-anonymous contract: signup must never be hit")
  }

...
final class DirectMessageServiceCrossPostTests: XCTestCase {
    ...
    private func makeHandler(
        ...
        slackResponse: (status: Int, body: Data) = (
            200,
            Data(#"{"ok":true,"ts":"1700000000.000100","channel_id":"C12345","error":null,"retry_after_seconds":null}"#.utf8)
        ),
        linearResponse: (status: Int, body: Data) = (
            200,
            Data(#"{"ok":true,"issue_id":"issue-uuid-1","identifier":"LEA-42","url":"https://linear.app/leaf/issue/LEA-42","error":null}"#.utf8)
        )
        ...
        return { request, body in
            ...
            case "/functions/v1/slack_post":
                ...
                return (HTTPURLResponse(url: request.url!, statusCode: slackResponse.status, httpVersion: nil, headerFields: nil)!,
                        slackResponse.body)
            case "/functions/v1/linear_create_issue":
                if let obj = try? JSONSerialization.jsonObject(with: body) as? [String: Any] {
                    sink.recordLinear(obj)
                }
                return (HTTPURLResponse(url: request.url!, statusCode: linearResponse.status, httpVersion: nil, headerFields: nil)!,
                        linearResponse.body)
            default:

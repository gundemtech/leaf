...
final class SupabaseClientInviteTokensTests: XCTestCase {
  ...
  /// the expected pubkey claim, then delegates all non-bootstrap calls to `tableHandler`.
  /// M-II: insertInviteToken now calls /functions/v1/insert_invite_token;
  /// listInviteTokens / markInviteTokenDeleted still call /rest/v1/invite_tokens.
  ...
  func testInsertInviteToken_PostsCorrectPayload_Returns201Echo() async throws {
    ...
    bootstrap { request, body in
      ...
      // M-II Edge response: single object { code, workspace_id, created_at }
      let respBody = """
    ...
    // M-II: Edge path does NOT set Prefer: return=representation
    XCTAssertTrue(
      (capturedHeaders["Prefer"] ?? "").isEmpty,
      "Edge body must not contain Prefer header")
    ...
    // M-II: created_by_pubkey is NOT sent to Edge fn (server extracts from JWT)
    XCTAssertNil(
      parsed["created_by_pubkey"],
      "Edge body must not contain created_by_pubkey")
    ...
    XCTAssertNotNil(parsed["created_at_ms"] as? Int, "created_at_ms must be present")
  }

...
final class SubmitToWaitlistTests: XCTestCase {
  ...
  func testSubmitToWaitlist_201Created_ReturnsSuccess() async throws {
    MockURLProtocol.handler = { request, _ in
      // M-II: Edge Function path, not PostgREST
      XCTAssertEqual(request.url?.path, "/functions/v1/submit_waitlist")
      XCTAssertEqual(request.httpMethod, "POST")
      let resp = HTTPURLResponse(
        url: request.url!, statusCode: 201, httpVersion: nil, headerFields: nil)!
      // M-II Edge response: single object { email, source, created_at }
      return (
        resp,
        Data(
          #"{"email":"alice@example.com","source":"upgrade_modal","created_at":"2026-05-23T10:00:00Z"}"#
            .utf8)
      )
    }
  ...
  // M-II: Edge returns 201 only (not 204); 204 test replaced by 200 fallback test
  func testSubmitToWaitlist_200OK_ReturnsSuccess() async throws {
    MockURLProtocol.handler = { request, _ in
      let resp = HTTPURLResponse(
      ...
      return (
        resp,
        Data(
          #"{"email":"bob@example.com","source":"upgrade_modal","created_at":"2026-05-23T10:00:00Z"}"#
            .utf8)
      )
    }
    let client = makeClient(session: makeSession())
    ...
    if case .failure(let err) = result {
      XCTFail("Expected success on 200; got \(err)")
    }
  }
  ...
  func testSubmitToWaitlist_409Conflict_ReturnsSuccess() async throws {
    MockURLProtocol.handler = { request, _ in
      ...
      return (
        resp,
        Data(
          #"{"code":"23505","message":"duplicate key value violates unique constraint \"waitlist_pkey\""}"#
            .utf8)
      )
    }
    let client = makeClient(session: makeSession())
  ...
  func testSubmitToWaitlist_EmptyEmail_FailsWithoutHTTPCall() async throws {
    ...
    MockURLProtocol.handler = { _, _ in
      ...
      return (
        HTTPURLResponse(
          url: URL(string: "x")!, statusCode: 201, httpVersion: nil, headerFields: nil)!, Data()
      )
    }
    let client = makeClient(session: makeSession())
  ...
  func testSubmitToWaitlist_BodyContainsLowercasedEmailAndSource() async throws {
    ...
    XCTAssertNil(
      capturedAuth, "submitToWaitlist must NOT send Authorization header — anonymous flow")

    guard let body = capturedBody,
      let json = try JSONSerialization.jsonObject(with: body) as? [String: Any]
    else {
      XCTFail("Body not captured / not JSON")
      return
    }
    XCTAssertEqual(
      json["email"] as? String, "alice@example.com",
      "email must be trimmed + lowercased before send")
    XCTAssertEqual(json["source"] as? String, "test_source")
    // M-II: created_at_ms must be present for idempotency hash
    XCTAssertNotNil(json["created_at_ms"] as? Int, "created_at_ms must be present")
  }
  ...
  func testSubmitToWaitlist_DefaultSourceIsUpgradeModal() async throws {
    ...
    XCTAssertEqual(
      json["source"] as? String, "upgrade_modal",

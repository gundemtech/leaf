...
final class GitHubWarmSchedulerTests: XCTestCase {
    private let logger = Logger(subsystem: "tech.gundem.leaf.tests", category: "gh-warmsched")
    private var tempDir: URL!
    private var dbURL: URL!

    override func setUp() async throws {
        tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("leaf-gh-warm-sched-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeWarmCollector() throws -> (Database, GitHubWarmCollector) {
        let db = try Database.openForWrite(at: dbURL, config: .weakDefaults, encryption: .deterministicTest)
        let collector = GitHubWarmCollector(
            database: db,
    ...
    func testStartStopIsIdempotent() async throws {
        ...
        let s = GitHubWarmScheduler(collector: c, intervalSec: 60, logger: logger)
        await s.start()
    ...
    func testStopIsPromptEvenWithLongInterval() async throws {
        let (_, c) = try makeWarmCollector()
        let s = GitHubWarmScheduler(collector: c, intervalSec: 9_999, logger: logger)
        await s.start()
    ...
    func testPerformTickRunsWithoutIntegration() async throws {
        ...
        let (_, c) = try makeWarmCollector()
        let s = GitHubWarmScheduler(collector: c, intervalSec: 900, logger: logger)
        await s.performTick()
    }
}

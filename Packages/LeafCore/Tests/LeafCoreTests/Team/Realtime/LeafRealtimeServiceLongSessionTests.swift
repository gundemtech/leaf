...
final class LeafRealtimeServiceLongSessionTests: XCTestCase {
  ...
  private func bootstrappedClient(
    clock: ClockBox,
    counter: CallCounter
  ) async throws -> SupabaseClient {
    ...
    let seedDir = FileManager.default.temporaryDirectory
      .appendingPathComponent("leaf-rtlong-\(UUID().uuidString)", isDirectory: true)
    ...
    try? seedStore.write(PersistedSession(refreshToken: "seed-rt", userID: userID, savedAtMs: 1))
  ...
  private func makeService(driver: RealtimeWebSocketDriver, supabase: SupabaseClient)
    -> LeafRealtimeService
  {
  ...
  func testRefreshIfNeeded_NoSession_SilentlySkips() async throws {
    ...
    await service.refreshIfNeeded()
    XCTAssertEqual(
      mock.outgoing.count, countBefore,
  ...
  private func waitForCondition(
    timeoutNanos: UInt64 = 5_000_000_000,

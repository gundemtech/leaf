...
final class ZoomStateMachineTests: XCTestCase {
    func testFirstObservationEmitsState() {
        ...
        let events = sm.observe(ZoomObservation(meetingState: .inMeeting, ownMeetingTopic: nil), nowMs: 1000, meetingTopicOptedIn: false)
    ...
    func testStateChangeEmitsStateEvent() {
        var sm = ZoomStateMachine()
        _ = sm.observe(ZoomObservation(meetingState: .notInMeeting, ownMeetingTopic: nil), nowMs: 1000, meetingTopicOptedIn: false)
        let events = sm.observe(ZoomObservation(meetingState: .inMeeting, ownMeetingTopic: nil), nowMs: 2000, meetingTopicOptedIn: false)
        XCTAssertEqual(events.count, 1)
        XCTAssertEqual(events[0].payload["meeting_state"], "in_meeting")
    }
    ...
    func testSubFieldOptInGatesTopicEmission() {
        ...
        var sm = ZoomStateMachine()
        let events = sm.observe(ZoomObservation(meetingState: .inMeeting, ownMeetingTopic: "Q1 review"), nowMs: 1000, meetingTopicOptedIn: false)
        XCTAssertEqual(events.count, 1)
        XCTAssertEqual(events[0].payload["event_kind"], "zoom_meeting_state_changed")
    }
    ...
    func testWithOptInTopicAndStateEmit() {
        var sm = ZoomStateMachine()
        let events = sm.observe(ZoomObservation(meetingState: .inMeeting, ownMeetingTopic: "Q1 review"), nowMs: 1000, meetingTopicOptedIn: true)
        ...
        XCTAssertEqual(events.first(where: { $0.payload["event_kind"] == "zoom_meeting_name_observed" })?.payload["meeting_topic"], "Q1 review")
    }
    ...
    func testPayloadHasNoAttendeesOrPasswordFields() {
        var sm = ZoomStateMachine()
        let events = sm.observe(ZoomObservation(meetingState: .inMeeting, ownMeetingTopic: "T"), nowMs: 1000, meetingTopicOptedIn: true)
        for ev in events {

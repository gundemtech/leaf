...
final class SupabaseClientPostInviteTests: XCTestCase {
    ...
    private func bootstrapResponses(adminPubkey: String,
    ...
    func testPostInvite_writesToInvitesTable_returnsToken() async throws {
        ...
        let expiresAt = Date(timeIntervalSince1970: 9999000000)
        ...
        let client = SupabaseClient(
            ...
            identity: { try Curve25519.KeyAgreement.PrivateKey(rawRepresentation: Data(repeating: 0x42, count: 32)) }
        )
    ...
    func testPostInvite_withOTP_includesHash() async throws {
        ...
        bootstrapResponses(adminPubkey: adminHex) { request, body in
            capturedBody = body
            let resp = HTTPURLResponse(url: request.url!, statusCode: 201, httpVersion: nil, headerFields: nil)!
            let respBody = #"[{ "token": "22222222-2222-2222-2222-222222222222", "workspace_id": "\#(workspaceID)", "expires_at": "2286-11-20T17:46:40Z" }]"#.data(using: .utf8)!
            return (resp, respBody)
        }

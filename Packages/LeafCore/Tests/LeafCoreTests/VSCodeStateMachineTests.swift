...
final class VSCodeStateMachineTests: XCTestCase {
    ...
    func test_identicalObservation_suppresses() {
        var sm = VSCodeStateMachine()
        let obs = VSCodeObservation(ideBundleID: "com.microsoft.VSCode", workspaceName: "leaf", fileBasename: "Foo.swift")
    ...
    func test_fileChange_emits() {
        var sm = VSCodeStateMachine()
        _ = sm.observe(.init(ideBundleID: "com.microsoft.VSCode", workspaceName: "leaf", fileBasename: "Foo.swift"), nowMs: 1_000)
        let events = sm.observe(.init(ideBundleID: "com.microsoft.VSCode", workspaceName: "leaf", fileBasename: "Bar.swift"), nowMs: 2_000)
        XCTAssertEqual(events.count, 1)
    ...
    func test_workspaceChange_emits() {
        var sm = VSCodeStateMachine()
        _ = sm.observe(.init(ideBundleID: "com.microsoft.VSCode", workspaceName: "leaf", fileBasename: "Foo.swift"), nowMs: 1_000)
        let events = sm.observe(.init(ideBundleID: "com.microsoft.VSCode", workspaceName: "leaf-docs", fileBasename: "Foo.swift"), nowMs: 2_000)
        XCTAssertEqual(events.count, 1)
    ...
    func test_perBundleIsolation_vscodeAndCursor() {
        ...
        let events = sm.observe(.init(ideBundleID: "com.todesktop.230313mzl4w4u92", workspaceName: "leaf", fileBasename: "Foo.swift"), nowMs: 2_000)
        XCTAssertEqual(events.count, 1)

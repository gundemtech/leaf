...
final class SupabaseClientCrossPostTests: XCTestCase {
  ...
  func testTriggerSlackPost_RequestBodyShape() async throws {
    MockURLProtocol.handler = wrapWithBootstrap { request, body in
      ...
      XCTAssertTrue(
        auth.hasPrefix("Bearer "), "Authorization header should be Bearer JWT, got: \(auth)")
      ...
      XCTAssertNotNil(
        parsed["slack_payload"] as? [String: Any], "slack_payload must be nested JSON object")
  ...
  // A13: idempotency_key body field removed; Idempotency-Key header injected
  // by performHTTP instead. Test renamed to reflect the new invariant.
  func testTriggerLinearCreate_LegacyIdempotencyKeyAbsentFromBody() async throws {
  ...
  func testTriggerLinearCreate_AssigneeNil_OmittedFromBody() async throws {
    MockURLProtocol.handler = wrapWithBootstrap { request, body in
      ...
      XCTAssertFalse(
        parsed.keys.contains("assignee_id"),

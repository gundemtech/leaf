...
final class SupabaseClientDirectMessagesTests: XCTestCase {
  ...
  func testSendDirectMessage_Success_ReturnsRow() async throws {
    MockURLProtocol.handler = wrapWithBootstrap { request, _ in
      ...
      XCTAssertEqual(request.url?.path, "/functions/v1/send_direct_message")
      XCTAssertEqual(request.httpMethod, "POST")
      // M-II: no Prefer header on Edge path
      XCTAssertNil(request.value(forHTTPHeaderField: "Prefer"))
      // M-II Edge response: single object { message_id, workspace_id, created_at }
      return (
        HTTPURLResponse(url: request.url!, statusCode: 201, httpVersion: nil, headerFields: nil)!,
        Data(
          """
          { "message_id": "11111111-1111-1111-1111-111111111111", "workspace_id": "ws-1", "created_at": "2026-05-14T10:00:00Z" }
          """.utf8)
      )
    }

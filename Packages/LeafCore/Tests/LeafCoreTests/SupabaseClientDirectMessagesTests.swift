...
final class SupabaseClientDirectMessagesTests: XCTestCase {
    ...
    private func wrapWithBootstrap(_ dmHandler: @escaping @Sendable (URLRequest, Data) -> (HTTPURLResponse, Data)) -> (@Sendable (URLRequest, Data) throws -> (HTTPURLResponse, Data)) {
        let jwt = makeJWT()
        let uid = userID

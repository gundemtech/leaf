...
final class DirectMessageServiceTests: XCTestCase {
  ...
  func testSend_FullPipeline_InsertsMirrorOutbound() async throws {
    ...
    MockURLProtocol.handler = { request, _ in
      ...
      // DM POST.
      if path == "/functions/v1/send_direct_message" && request.httpMethod == "POST" {
        return (
  ...
  func testSend_NotifyOff_SkipsAPNs() async throws {
    ...
    MockURLProtocol.handler = { request, _ in
      ...
      if path == "/functions/v1/send_direct_message" && request.httpMethod == "POST" {
        return (
          ...
          Data(
            """
            { "message_id": "\(messageID)", "workspace_id": "11111111-1111-1111-1111-111111111111", "created_at": "2026-05-14T10:00:00Z" }
            """.utf8)
        )
      }
      if path == "/functions/v1/apns_push" {
    ...
    XCTAssertFalse(
      apnsCalledBox.wasCalled, "apns_push should NOT have been called when notify=false")
  }
}

...
final class InviteTokenServiceTests: XCTestCase {
  ...
  func testGenerate_CreatesTokenServerSideAndMirrorsLocally() async throws {
    ...
    let token = try await service.generate(
      label: "Team kickoff", ttlSeconds: 86400, singleUse: false)
  ...
  func testGenerate_SingleUseSetsMaxUses1() async throws {
    bootstrap(invitePostHandler: { request, body in
      ...
      let resp = HTTPURLResponse(
        url: request.url!, statusCode: 201, httpVersion: nil, headerFields: nil)!
      let respBody = self.echoRow(
        code, expiresAt: self.fixedNow.addingTimeInterval(3600),
        maxUses: 1, label: nil)
      return (resp, respBody.data(using: .utf8)!)
    })
  ...
  func testDelete_MarksServerAndLocalMirror() async throws {
    ...
    bootstrap(
      invitePostHandler: { request, body in
        let parsed = (try? JSONSerialization.jsonObject(with: body)) as? [String: Any]
        let code = (parsed?["code"] as? String) ?? "LEAF-AAAA-BBBB-CCCC"
        let resp = HTTPURLResponse(
          url: request.url!, statusCode: 201, httpVersion: nil, headerFields: nil)!
  ...
  // Round-6 fix (commit 5950ced) — InviteTokenService.delete reordered to
  // local-mirror-first, server PATCH best-effort with `.noRowsAffected`
  // swallowed. Pre-fix, server-first ordering meant cron-swept rows or
  // legacy tokens never on server threw before the local UPDATE ran,
  // resurrecting «deleted» tokens on every refresh and trapping the user
  // in a delete-loop. These two tests lock the new behavior so future
  // refactors don't regress.

  func testDelete_ServerNoRowsAffected_LocalMirrorStillCommits() async throws {
    bootstrap(
      invitePostHandler: { request, _ in
        let resp = HTTPURLResponse(
          url: request.url!, statusCode: 201, httpVersion: nil, headerFields: nil)!
        let respBody = self.echoRow(
          "LEAF-AAAA-BBBB-CCCC",
          expiresAt: self.fixedNow.addingTimeInterval(86400),
          maxUses: nil, label: "Legacy")
        return (resp, respBody.data(using: .utf8)!)
      },
      invitePatchHandler: { request, _ in
        // Server returns 204 + Content-Range */0 → fromStatus-like
        // path in SupabaseClient throws `.noRowsAffected` (legacy
        // row never on server, or cron-swept between our refresh
        // and our delete attempt).
        let resp = HTTPURLResponse(
          url: request.url!, statusCode: 204, httpVersion: nil,
    ...
    let service = makeService()
    let token = try await service.generate(
      label: "Legacy", ttlSeconds: 86400, singleUse: false)

    // delete() must NOT throw — round-6 fix swallows .noRowsAffected.
    try await service.delete(code: token.code)

    // Local mirror was UPDATED with deletedAt even though server
    // PATCH said «zero rows touched». Critical to break the
    // resurrect-on-refresh loop dogfooding hit.
    let local = try db.fetchInviteToken(code: token.code)
    XCTAssertNotNil(
      local?.deletedAt,
      "local mirror must commit deletedAt even when server PATCH 0-rows"
    )
  }

  func testDelete_ServerErrorPropagates_LocalAlreadyCommittedFirst() async throws {
    bootstrap(
      invitePostHandler: { request, _ in
        let resp = HTTPURLResponse(
          url: request.url!, statusCode: 201, httpVersion: nil, headerFields: nil)!
        let respBody = self.echoRow(
          "LEAF-DDDD-EEEE-FFFF",
          expiresAt: self.fixedNow.addingTimeInterval(86400),
          maxUses: nil, label: "Boom")
        return (resp, respBody.data(using: .utf8)!)
      },
      invitePatchHandler: { request, _ in
        // Server 500 — non-conflict, non-`.noRowsAffected` error.
        let resp = HTTPURLResponse(
    ...
    let service = makeService()
    let token = try await service.generate(
      label: "Boom", ttlSeconds: 86400, singleUse: false)

    // The server error propagates so the caller sees the failure —
    // BUT the local UPDATE has already happened (local-first ordering),
    // so the user-visible state on next refresh is convergent.
    do {
      try await service.delete(code: token.code)
      XCTFail("expected SupabaseError for 500")
    } catch is SupabaseError {
      // pass — server error surfaced for diagnostics
    }

    let local = try db.fetchInviteToken(code: token.code)
    XCTAssertNotNil(
      local?.deletedAt,
      "local commits BEFORE server PATCH — even an exploding server doesn't roll it back"
    )
  }
}

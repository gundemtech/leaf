...
final class SupabaseClientTeamEventsTests: XCTestCase {
  ...
  private func wrapWithBootstrap(
    _ handler: @escaping @Sendable (URLRequest, Data) -> (HTTPURLResponse, Data)
  ...
  func testSendTeamEvent_Success_ReturnsRow() async throws {
    MockURLProtocol.handler = wrapWithBootstrap { request, _ in
      ...
      XCTAssertEqual(request.url?.path, "/functions/v1/send_team_event")
      XCTAssertEqual(request.httpMethod, "POST")
      // M-II: no Prefer header on Edge path
      XCTAssertNil(request.value(forHTTPHeaderField: "Prefer"))
      // M-II Edge response: single object { event_id, workspace_id, created_at }
      return (
        HTTPURLResponse(url: request.url!, statusCode: 201, httpVersion: nil, headerFields: nil)!,
        Data(
          """
          { "event_id": "evt-1", "workspace_id": "ws-1", "created_at": "2026-05-15T10:00:00Z" }
          """.utf8)
      )
    }
  ...
  func testSendTeamEvent_BodyContainsAllFieldsIncludingExpiresAt() async throws {
    ...
    MockURLProtocol.handler = wrapWithBootstrap { [captured] request, body in
      // M-II: capture on Edge path (not PostgREST)
      if request.url?.path == "/functions/v1/send_team_event" {
        captured.set(body)
      }
      // M-II Edge response: single object { event_id, workspace_id, created_at }
      return (
    ...
    // M-II: event_id NOT sent to Edge (server generates its own); sender_pubkey removed
    XCTAssertNil(json["event_id"], "Edge body must not contain event_id")
    XCTAssertNil(json["sender_pubkey"], "Edge body must not contain sender_pubkey")

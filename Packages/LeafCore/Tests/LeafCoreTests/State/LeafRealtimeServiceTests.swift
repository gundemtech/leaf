...
final class LeafRealtimeServiceTests: XCTestCase {
  ...
  private let url = URL(
    string: "wss://test.supabase.co/realtime/v1/websocket?apikey=key&vsn=1.0.0")!
  ...
  private func installAuthBootstrap() async throws -> SupabaseClient {
    ...
    let seedDir = FileManager.default.temporaryDirectory
      .appendingPathComponent("leaf-rt-\(UUID().uuidString)", isDirectory: true)
    ...
    try? seedStore.write(PersistedSession(refreshToken: "seed-rt", userID: uid, savedAtMs: 1))
  ...
  nonisolated static func teamEventPlaintextMatching(
    _ input: EncryptedTeamEventInput,
    claimedSender: String,
    claimedWorkspaceID: String? = nil,
    eventID: String = "ev-X",
    kind: String = "commit_authored"
  ) -> TeamEventPlaintext {
    TeamEventPlaintext(
      eventID: eventID,
  ...
  nonisolated static func dmPlaintextMatching(
    _ input: EncryptedDirectMessageInput,
    claimedSender: String,
    claimedWorkspaceID: String? = nil,
  ...
  private nonisolated func defaultDirectMessageDecryptor()
    -> LeafRealtimeService.DirectMessageDecryptor
  {
    let sender = self.pubkey
  ...
  private func makeService(
    ...
    activeWorkspaceStore: ActiveWorkspaceStore = ActiveWorkspaceStore(
      userDefaults: TestUserDefaults.make()),
  ...
  func testDispatch_TeamEventSenderMismatch_C2Skip() async throws {
    ...
    try await Task.sleep(nanoseconds: 100_000_000)
    XCTAssertEqual(
      teamStub.received.count, 0, "Mismatched sender plaintext claim must be C2-skipped")
    await service.suspend()
  }
  ...
  func testDispatch_DirectMessageInsert_TriggersCrossPostFetch() async throws {
    ...
    XCTAssertGreaterThanOrEqual(
      fetchCount.value, 1, "cross_post_log fetch must fire after DM insert")
    await service.suspend()
  }
  ...
  func testSuspend_TransitionsToSuspended_CancelsDispatch() async throws {
    ...
    XCTAssertTrue(
      service.state == .connecting || service.state == .connected,

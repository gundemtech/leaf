...
extension DatabaseMigrator {
    public mutating func registerMigration014DetectionTables() {
        registerMigration("014_detection_tables") { db in
            try Self.m014CreateDecisions(db)
            try Self.m014CreateOpenQuestions(db)
            try Self.m014CreateBlockers(db)
            try Self.m014CreateWhereStoppedLog(db)
            try Self.m014CreateDetectorOffsets(db)
        }
    }

    // MARK: - decisions

    private static func m014CreateDecisions(_ db: GRDB.Database) throws {
        try db.execute(
            sql: """
    ...
    private static func m014CreateOpenQuestions(_ db: GRDB.Database) throws {
        try db.execute(
            sql: """
    ...
    private static func m014CreateBlockers(_ db: GRDB.Database) throws {
        try db.execute(
            sql: """
    ...
    private static func m014CreateWhereStoppedLog(_ db: GRDB.Database) throws {
        try db.execute(
            sql: """
    ...
    private static func m014CreateDetectorOffsets(_ db: GRDB.Database) throws {
        try db.execute(
            sql: """

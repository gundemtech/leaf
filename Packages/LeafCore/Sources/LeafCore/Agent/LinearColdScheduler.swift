...
/// Phase Track-3 D1 — 4am local anchor scheduler for LinearColdCollector.
///
/// On start: reads `linear:cold:<wid>` offset from collector_offsets. If absent
/// or `now - lastColdRunMs > 24h`, performs catch-up tick immediately. Then
/// loops: compute next local 4am (via Calendar.current), Task.sleep until then,
/// perform tick, repeat. If the Mac is asleep across 4am, Task.sleep continues
/// on wake — tick fires at first wake-after-4am (accepted "off-hours rough"
/// behavior per spec §5).
public actor LinearColdScheduler {
    ...
    public func start() {
        ...
        logger.info("LinearColdScheduler started")
    }
    ...
    public func stop() async {
        ...
        await task?.value
        task = nil
        logger.info("LinearColdScheduler stopped")
    }
    ...
    private func runLoop() async {
        ...
        if let wid = workspaceIDProvider() {
            let offset = try? database.readOffset(
                collectorID: CollectorID.linearColdPolling, sourceID: "linear:cold:\(wid)")

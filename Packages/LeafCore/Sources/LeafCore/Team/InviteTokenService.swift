...
public struct InviteTokenService: Sendable {
    ...
    public func generate(
        ...
        let expiresAt: Date? = ttlSeconds > 0
    ...
    /// Soft-delete a token. Server PATCHes `deleted_at`; mirror UPDATE follows.
    /// Idempotent — re-deleting already-deleted token is a no-op (server + local
    /// both overwrite `deleted_at` to current timestamp).

...
public struct QueryEngine: Sendable {
    ...
    private static func fetchCurrentApp(in db: GRDB.Database) throws -> String? {
        try String.fetchOne(
            db,
            sql: """
    ...
    private static func fetchCurrentBranch(in db: GRDB.Database) throws -> String? {
        try String.fetchOne(
            db,
            sql: """
    ...
    /// type in substrate).
    private static func fetchCurrentFile(in db: GRDB.Database) throws -> String? {
        try String.fetchOne(
            db,
            sql: """
    ...
    private static func fetchInProgressLinearTicket(in db: GRDB.Database) throws -> LinearTicketRef? {
        try Row.fetchOne(
            db,
            sql: """
    ...
    private static func fetchLastCommit(in db: GRDB.Database) throws -> CommitRef? {
        try Row.fetchOne(
            db,
            sql: """
                    SELECT ts,
    ...
    private static func fetchCurrentOpenQuestions(in db: GRDB.Database) throws -> [OpenQuestionView] {
        try Row.fetchAll(
            db,
            sql: """
    ...
    private static func fetchCurrentBlockers(in db: GRDB.Database) throws -> [BlockerView] {
        try Row.fetchAll(
            db,
            sql: """
    ...
    private static func fetchWhereStopped(nowMs: Int64, in db: GRDB.Database) throws -> WhereStoppedOutput? {

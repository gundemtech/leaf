...
**2026-05-07 — Phase 5.5.B flow rewrites landed на `feature/phase-5-5-B-flow-rewrites`** (stack on `feature/phase-5-5-A-foundation` off main, alpha.11 baseline). Substrate-плюс-UX layer: `ShareTemplate` + `ClipboardMatcher` LeafCore helpers; `InviteURLHandler` Leaf-target glue (deep-link routing + NSPasteboard probe); `InviteService.generateInvite(joinCode:)` + `InviteAcceptService.fetchInvite(inviteURL:)` overloads (`inviteNotFound` → `inviteAlreadyConsumed` UX remap); 2 new public `Database` wrappers (`insertPendingInvite` / `updatePendingInviteStatus`); `InviteOutboxReader` пишет `pending_invites`; `InviteAcceptReader.fetch(inviteURL:)` + persisted invitee displayName + auto-prefill OTP в otpEntry state; new SwiftUI views (`ShareTemplateButton` / `CreateTeamStepView` / `JoinTeamStepView` / `WaitingForInviteView`); full rewrites `GenerateInviteSheet` (Picker Paste/Template + deep-link output) + `AcceptInviteSheet` (auto-fill from URL/clipboard + simplified verification flow); `Info.plist` `CFBundleURLTypes` registration; LeafApp `.onOpenURL` + `.onChange(of: scenePhase)` clipboard-on-foreground auto-fetch wiring; `OrgReader.State: Equatable` for `.onChange(of:)`. **989 SPM tests** total (971 + 18 new). 5/5 xcodebuild schemes green. Stage 6 review: 2 Important + 4 Minor — 4 fixes shipped в `cf64ba4` (scenePhase wiring, ShareTemplate empty-name greeting, GenerateInviteSheet single pasteboard read, drop unused LeafError cases). **Не merged в main** — stack под 5.5.C (admin pending list + manual Refresh + Re-share/Revoke). Tactical: `.claude/plans/phase-5-5-B.md`.
...
- **Phase 5.5 stack in flight.** 5.5.A (substrate — `JoinCode` + `InviteURL` + M010 `pending_invites` + `PendingInvitesStore`, 971 tests baseline) + 5.5.B (UX surface — deep-link routing + scenePhase clipboard auto-fetch + `.team` three-way + sheet rewrites + JoinCode/URL service overloads, 989 tests) обе ship'нули в свои feature branches. Stack под 5.5.C (admin pending list + manual Refresh + Re-share/Revoke).

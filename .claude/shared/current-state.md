...
- **AI Coworker P3** (Escalation §10): кластер 5 — bodies-эскалация UX + reverse audit log + prompt-injection defense (§8 п.3/4). + Slack cross-linking (B persist + C semantic) + bare-PR-number normalizer (re-enables github_pr `cross_link_fact`); + P1/P2 deferred детектор-excerpts на escalation path. Переиспользует P1/P2 `WorkFactGatherer`+boundary.

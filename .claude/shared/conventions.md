...
- **R1** — Phase-done ⟺ `just preflight` green (leak-guard + check-tokens + check-migrations + gitleaks + build-all 5 схем + SPM-тесты). Verified, не «вроде работает». **Merge-gate: локальный preflight НЕОБХОДИМ, но НЕ достаточен — PR мержится только когда CI-чеки (`ci` + `leak-guard`) на ветке/PR зелёные.** Локально-зелёно ≠ CI-зелёно: чистый раннер (Swift 6 strict, без DerivedData-кэша) ловит то, что incremental-сборка маскирует — data-race, flaky-timing. Дождись зелёного CI перед `gh pr merge`. *(auto: preflight + CI required-checks)*
...
- **R4** — Trunk-based-lite: `dev` = единственный транк; `main` = только release-теги; фича-ветки короткие, удаляются СРАЗУ после merge (`gh pr merge --squash --delete-branch` + worktree чистится); одна phase = одна сессия. **Direct push в транк только `chore`/`ci`/`docs`/`release`. Всё остальное (`fix`/`feat`/`test`/`refactor`/`perf`) — через feature-ветку + PR в `dev`, merge по R1 merge-gate (зелёный CI).** Перед закрытием ветки: PR смержен, CI был зелёный, локальная ветка + remote удалены, `dev` синхронизирован (`git pull --ff-only`).

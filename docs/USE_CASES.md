...
Alex: "what was Sasha doing today?"

> Sasha today — OAuth migration (LEAF-127), 3 commits in `src/auth/refresh.ts`. Stopped halfway through rotation logic. Open question in #leaf-architecture about token rotation policy.

### Use case 3 — Production bug at night

Maya: "what changed in /payments over the last 2 weeks?"

> On 28 April Sasha migrated to Stripe API v2024-09 (LEAF-203). He noted "webhook signatures use a new format — needs to be tested". No commits to the webhook handler since then. Worth checking there.

### Use case 4 — Decision archaeology three weeks later

"Why did we move OAuth refresh to the server?"

> Decision on 22 April. In LEAF-127's description Sasha wrote: 'client-side refresh = refresh token in localStorage, security concern'. In #leaf-architecture this was discussed with Alex → server-side via httponly cookie.

### Use case 5 — Preparing for code review

Alex: "what should I know before reviewing PR #142?"

> PR #142 — persistent memory for MCP queries (LEAF-198). Sasha worked on it for 3 days, 7 commits. **Important:** on Tuesday in #leaf-architecture he wrote that he'd chosen LRU eviction at 1000 entries — but this wasn't discussed or agreed with you.

### Use case 6 — Slack bot for the whole team

A new team member: `/leaf what did we decide about backups and why?`

> Decision on 3 March, initiator — Sasha. S3 backups (incremental hourly + full daily) instead of managed RDS — cost savings + portability. Implementation by Alex, 4–5 March in `infra/backup/`.

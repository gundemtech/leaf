...
**T6 status pass (2026-05-21):** 3 carries closed (C-1 hybrid pills via SurfacePillKind + PillFamily + family-aware substrate refactor; C-2 InsightsReader error retention + HomeView partial render; C-3 EXTENDED to 5-cell wide + Grid 2×3 narrow). See T6 spec `docs/superpowers/specs/2026-05-21-track-9-T6-today-hybrid-pills.md`.

**From P3 (TODAY block):**

- **C-1 Hybrid surface pills** — **[RESOLVED T6 — commits `fabcfbda` + `11f87720` + `4ed3ddaa`]** `SurfacePillKind` enum (`.captureTime` / `.actionNoun`) + `PillFamily` enum (13 cases: 10 capture-time + 3 Layer B) shipped on public LeafCore. Moat: `ProdTodayPillFamilyMap.bundlesByFamily` per-family bundle ID set; `queryPills()` refactored to `timeInApp` aggregation for capture surfaces + new helpers `queryMeetingTimeSec` (Calendar) / `queryLinearIssuesCompletedCount` / `queryPRsMergedCount` / `querySlackMessagesAuthoredCount`. UI: `TodayBlock.pillLabel(for:)` switches on kind ("Xh Ym" vs "N"). Balance-by-kind cap: top 5 capture + top 3 action-noun = ≤ 8 total. Spec: `docs/superpowers/specs/2026-05-21-track-9-T6-today-hybrid-pills.md`.
- **C-2 Error-state last-known retention** — **[RESOLVED T6 — commits `b8074e0e` + `47db6011`]** `InsightsReader.State.error(message:, lastKnown: InsightsSnapshot?)` associated value added. `refresh()` maintains instance field `lastKnownSnapshot` updated on `.loaded` transitions; catch path reads instance field (race-safe across overlapping refreshes — initial commit used local-scope capture which lost snapshot on rapid double-tap; review-fix commit migrated to instance field). `HomeView` `.error` branch: lastKnown nil → banner-only / non-nil → banner + `HomeContent(snapshot: lastKnown)`. Phase 8.3 spec §6 MS-5 contract closed.
- **C-3 Narrow-window wrap fallback** — **[RESOLVED P9 — commit `0dbadcd7`; EXTENDED T6 — commit `06a874f2`]** P9 shipped `ViewThatFits` wide-4-cell + narrow-2×2-grid; T6 extended to wide-5-cell (`switchCount` surfaced) + narrow SwiftUI `Grid` 2-col × 3-row row-major (5 cells fill top-left → bottom-middle).
- **C-4 DateFormatter caching** — **[RESOLVED P9 — commit `0dbadcd7`]** `TodayBlock.sectionLabel` allocates a `DateFormatter` per render. Tiny cost in practice but trivial fix: `static let` cached formatter (note: `setLocalizedDateFormatFromTemplate` is locale-aware, so cache by locale or accept en_US-only).
- **C-5 YouNowBlock LocalAppsStore reactivity gap (P4 carry)** — **[DEFERRED → Track-9 stub `docs/superpowers/specs/2026-05-19-track-9-substrate-enrichment.md`]** `YouNowBlock` constructs `LocalAppsStore()` as `@StateObject` (per-view instance). UserDefaults reads are consistent across instances, but `objectWillChange` fires only on the Settings instance when user toggles a bundle enabled flag. Home's Resume CTA won't reactively hide/show until the next `InsightsReader.refresh()` tick. Bounded by refresh cadence; fix = inject a single shared `LocalAppsStore` from app root via `.environmentObject(...)` and consume via `@EnvironmentObject`.
- **C-6 YOU·NOW inMeeting tap → Calendar `ical://` deep-link (P4 carry)** — **[DEFERRED → Track-9 stub `docs/superpowers/specs/2026-05-19-track-9-substrate-enrichment.md`]** master §3.4 says inMeeting click "opens Calendar app to the meeting (if Cal-linked)". P4 ships as no-op (tap gate confined to `.away` for Resume CTA). Calendar deep-link needs `ical://` URL scheme research (start time + UID lookup) + tap modifier extension to also gate on `.inMeeting`. Pairs naturally with substrate gap C-9 so the meeting render carries enough info to build a deep-link.
- **C-7 YouNowBlock accessibility — Button wrap on .away (P4 carry)** — **[RESOLVED P9 — commit `f8464da2`]** tap target is `.contentShape(Rectangle()).onTapGesture` (gated to `.away`), not a wrapping `Button`. VoiceOver may not announce the tappable affordance. Add explicit `Button` wrapper around the `.away` row or apply `.accessibilityAddTraits(.isButton)` on the card when CTA-eligible. Pairs with P9 accessibility audit pass. **(P9 chose the `.accessibilityAction(named: "Resume")` path over outer Button wrap to avoid nested-Button anti-pattern with inner Resume CTA Button per a11y audit BLOCKER finding.)**
- **C-8 Resume CTA branch-deletion staleness (P4 carry)** — **[DEFERRED → Track-9 stub `docs/superpowers/specs/2026-05-19-track-9-substrate-enrichment.md`]** if `lastLinearID` was extracted from a now-deleted branch, Resume CTA still renders with stale ID. v1.1 — validate via `LinearIDExtractor` registry refresh or `git branch --list` check before showing CTA. Not blocking ship.
- **C-9 YouNowMeeting substrate enrichment (P4 carry, cross-track)** — **[DEFERRED → Track-9 stub `docs/superpowers/specs/2026-05-19-track-9-substrate-enrichment.md`]** `ProdInsights+YouNowState.swift` hard-codes `meetingTitle: nil`, `meetingEndsAtMs: nil`, and `meetingSource` only `.eventKit` (Zoom never fires). Substrate enrichment needed: read meeting title from `meeting_state_entered` event payload (per ADR-010 allow-list), fetch `endsAtMs` from same payload, merge `zoom_meeting_started` snapshots into source `.both` when overlap window matches. Separate substrate track — not pure P4.

**From P5 (WITH YOU ON THIS):**

- **C-10 WithYouOnThisBlock empty-state CTA missing N count.** **[DEFERRED → Track-9 stub `docs/superpowers/specs/2026-05-19-track-9-substrate-enrichment.md`]** Spec §4.3 calls for "→ Team (N active elsewhere)" where N = teammates active anywhere who don't match my task. P5 ships CTA as "→ Team" without count. Resolution requires either (a) new `totalActiveTeammates` deriver, or (b) plumbing teammate list through `InsightsSnapshot` (privacy surface expansion — list of all teammates, not just matches). Phase 5.4 enrichment. File: `Leaf/Views/Window/Home/Blocks/WithYouOnThisBlock.swift:emptyState`.
- **C-11 WithYouOnThisBlock offline / stale footer absent.** **[DEFERRED → Track-9 stub `docs/superpowers/specs/2026-05-19-track-9-substrate-enrichment.md`]** Spec §4.3 calls for muted footer "Team data stale ({lastSync} ago). Reconnecting…" when relay disconnected OR last `presence_history` sync > 10 min. No relay status signal in `InsightsSnapshot` today. P9 / Phase 5.6 carry — pairs with relay status plumbing.
- **C-12 Row tap routes to Team tab without teammate selection.** **[DEFERRED → Track-9 stub `docs/superpowers/specs/2026-05-19-track-9-substrate-enrichment.md`]** Spec §4.3 "Click row → opens teammate detail in Team tab" — Team tab has no per-teammate detail screen. P5 ships row tap as `windowState.section = .team`. Resolution = Team-tab teammate detail screen + `RouteCoordinator.pushTeam(memberID:)`. Track-9 / separate feature.
- **C-13 TeammateMatch.durationSec hardcoded 0 in substrate.** **[DEFERRED → Track-9 stub `docs/superpowers/specs/2026-05-19-track-9-substrate-enrichment.md`]** `SameTaskMatcher.makeMatch` sets `durationSec: 0` unconditionally (`Packages/LeafCore/Sources/LeafCore/Insights/SameTaskMatcher.swift:73`). UI does not surface a "duration on task" field today (row line 2 uses `lastActivityAtMs` "ago" relative time). Substrate enrichment (compute duration from earliest task-matching snapshot per teammate) is Phase 5.4 / Track-9 territory.

**From P6 (INBOX):**

- **C-14 Search debounce / SQL re-fetch when cardinality grows.** **[DEFERRED → Track-9 stub `docs/superpowers/specs/2026-05-19-track-9-substrate-enrichment.md`]** Phase 8.6 ships pure in-memory filter on snapshot (≤100 items per 14-day cutoff). When Layer B Phase 4.8/4.9 lights up `reviewRequest` / `commentOnMyWork` / `mention` sources, cardinality may exceed 1000 items in busy team accounts. At that point view-side `String.contains` may visibly stutter on per-keystroke filter. Resolution = either (a) introduce 150ms `Task` cancellation debounce on `searchQuery` change, or (b) push filter/query down into `InsightsReader.refresh(inboxFilter:inboxQuery:)` with per-keystroke SQL re-fetch (substrate already supports the args). Carry to Track-9 or Phase 4.9 wire-up — whichever first exceeds the threshold. File: `Leaf/Views/Window/Home/Blocks/InboxBlock.swift:filteredItems`.
- **C-15 `RouteCoordinator.openURL(_:URL)` extraction if other blocks adopt external-URL navigation.** **[DEFERRED → Track-9 stub `docs/superpowers/specs/2026-05-19-track-9-substrate-enrichment.md`]** P6 ships `NSWorkspace.shared.open(_:)` direct call in `InboxItemRow.handleTap`. WHERE STOPPED (P7) may also need external URL open (linking to commit / issue). If so, extract a coordinator method to centralise: telemetry hook, error surface (URL invalid / app not found), URL allow-list per privacy review. Track-9 candidate. File: `Leaf/Views/Window/Home/Blocks/InboxItemRow.swift:handleTap`.
- **C-16 `InboxItem.sourceURL` nil for D3-derived items.** **[DEFERRED → Track-9 stub `docs/superpowers/specs/2026-05-19-track-9-substrate-enrichment.md`]** Open questions + blockers derived via `DetectorPipeline` currently land with `sourceURL == nil` because D3 detection tables don't preserve the originating Slack / Linear / GitHub URL. Result: rows render but are inert (`.disabled(true)`). Resolution = substrate enrichment — propagate `context_ref` (slack_thread_ts / linear_issue_ref / github_pr_ref) into the detection tables and synthesise a `sourceURL` at deriver boundary. Track-9 / Phase 4.8/4.9 backend pairing. File: `Packages/LeafCorePrivate/Prod/Insights/ProdInsights+InboxItems.swift:queryOpenQuestionsForMe / queryBlockersAffectingMe`.
- **C-17 `InboxBlock.filteredItems` substring-match unit test missing.** **[RESOLVED P9 — commits `52f4483e` + `835141c7`]** P6 spec §8.2 deferred view-state tests, but the `filteredItems` computed property has non-trivial semantics (case-insensitivity via `lowercased()`, whitespace trimming via `trimmingCharacters(.whitespacesAndNewlines)`, multi-field OR-match on `title || sourceMeta`, empty-query short-circuit). A 5-line XCTest against synthetic `[InboxItem]` fixtures would lock the spec §3.2 contract without invoking SwiftUI. P9 polish (or split as new `InboxBlockTests.swift`). File: `Leaf/Views/Window/Home/Blocks/InboxBlock.swift:filteredItems`.
- **C-18 Empty / no-match states share the same icon.** **[RESOLVED P9 — commit `835141c7`]** Both `emptyDataState` ("All clear.") and `noMatchState` ("No matches.") render `LeafIcons.brand.leaf`. Semantically different — one is "inbox is genuinely empty" (positive), the other is "filter/search hides things" (neutral, user-actionable). Visually identical icon confuses scan. Resolution = either (a) introduce a filter / magnifying-glass-slash glyph for no-match, or (b) accept that the CTA difference ("nothing" vs "Clear filters") carries the semantic load. P9 polish. File: `Leaf/Views/Window/Home/Blocks/InboxBlock.swift:emptyDataState / noMatchState`.
- **C-19 `InboxItemRow.severityWord` hardcoded English.** **[DEFERRED → Track-9 stub `docs/superpowers/specs/2026-05-19-track-9-substrate-enrichment.md`]** Returns "Urgent" / "Needs response" / "Informational" inline. Consistent with broader Track-7/8 known-limitation (Localizable.strings extraction pending — see C-X of Track-7 wrap). When localization track lands, this is a touchpoint. Track-9 / Localization track. File: `Leaf/Views/Window/Home/Blocks/InboxItemRow.swift:severityWord`.
- **C-20 WHERE STOPPED Line 2 dedicated last-commit subject (P7 carry).** **[DEFERRED → Track-9 stub `docs/superpowers/specs/2026-05-19-track-9-substrate-enrichment.md`]** Master spec §4.5 mockup specifies a 2-line populated body: Line 1 = excerpt + file ref, Line 2 = "last commit subject (60 char cap) if commit ≤ 4h ago". Phase 8.1 substrate `WhereStoppedSnapshot.excerpt` already prioritises commit subject when commit-based, covering Line 1 inline for the common case. Dedicated Line 2 requires a separate SQL helper (`recentLastCommit(maxAgeMs:)`) on `DerivedInsights` plus a `LeafCorePrivate` query against `events WHERE event_kind = 'git_commit' AND timestamp >= now - 4h ORDER BY timestamp DESC LIMIT 1`. Resolution = substrate extension in Track-9 / Phase 4.9. Until then, populated render falls back to single-line `excerpt`. File: `Packages/LeafCore/Sources/LeafCore/Insights/DerivedInsights.swift`.
- **C-21 WHERE STOPPED `anchorEventId → file path:line` resolution (P7 carry).** **[DEFERRED → Track-9 stub `docs/superpowers/specs/2026-05-19-track-9-substrate-enrichment.md`]** Master spec §4.5 mockup illustrates "Track-7 P5 polish · WorkStateCard.swift:142" — a hybrid label combining excerpt with the source event's file path and line number. Current `WhereStoppedSnapshot.anchorEventId: Int64?` carries only the event id; resolving to a path:line pair requires a second-pass lookup against `events.payload_json` (extract file_path) and line-number capture in the originating collector (Layer A AX or FSEvents — not currently retained in payload). Substrate enrichment pair: (a) add file_path+line to `events` payload allow-list for relevant kinds, (b) add `WhereStoppedSnapshot.anchorFilePath: String?` + `.anchorLine: Int?` fields. Resolution = Track-9 substrate sweep paired with C-20 backend fetch. File: `Packages/LeafCorePrivate/Prod/Detection/ProdWhereStoppedDeriver.swift`.
- **C-22 `formatRelative` helper duplicated across Home blocks (P7 carry).** **[RESOLVED P9 — commits `cd6a1c52` + `d3669782` + `0b80dfc9` + `2a698f11`]** Phase 8.7 `WhereStoppedBlock` inlines its own `formatRelative(_ deltaMs:Int64, nowMs:Int64) -> String` with bucket ladder (`now` / `Nm ago` / `Nh ago` / `yesterday` / `N days ago` / `MMM d`). Phase 8.5 `WithYouOnThisBlock` ships an equivalent helper with identical buckets. Two divergent copies of the same logic now live in Home blocks; future bucket-rule changes (e.g., localization carry C-19) require touch in both. Pairs with P5 review NIT-3 helper-unification deferral. Resolution = extract `HomeRelativeTimeFormatter.swift` shared helper in Track-9 / P9 polish. Files: `Leaf/Views/Window/Home/Blocks/WhereStoppedBlock.swift:formatRelative`, `Leaf/Views/Window/Home/Blocks/WithYouOnThisBlock.swift:formatRelative`.
- **C-23 Analytics surface real content (P8 carry, Track-9 follow-up).** **[RESOLVED T9 — see Track-9 T9 SHIPPED commits on `feature/track-9-substrate`, spec `docs/superpowers/specs/2026-05-21-track-9-T9-analytics-ui.md`]** Track-9 T9 replaces Phase 8.8 P8 40-LOC placeholder with full 6-block surface (WeekChipStrip + DailyFocusedChart + StreaksCard + PeakHourCallout + WoWDeltaCallout + TopToolsPlaceholder) consuming Track-9 T4 `WeeklyMetrics` substrate. `InsightsSnapshot.weeklyMetrics: WeeklyMetrics = .empty` defaulted-init field + `InsightsReader.refresh()` 23rd sequential SQL call. State machine mirror HomeView Track-8 P3 + T6 last-known retention. First SwiftUI Charts usage in codebase (single-Y-axis pragmatic chart given macOS 14 Charts API limitation — see T9 spec §1.1 (5) deviation, real dual-axis carry post-Track-9 C-44). `isLogicallyEmpty` predicate в AnalyticsContent (T9 spec §1.1 (6) deviation — substrate writes real `dayStartMs` even on cold DB). T6 SurfacePill discriminator coupling kept TopToolsCard as honest "Coming soon" placeholder (T9 spec §1.1 (3), carry C-38). Files: `Leaf/Views/Window/Analytics/AnalyticsView.swift` + `Leaf/Views/Window/Analytics/AnalyticsContent.swift` + 6 `Leaf/Views/Window/Analytics/Blocks/*.swift` + `Packages/LeafCore/Sources/LeafCore/Insights/InsightsSnapshot.swift` + `Leaf/Models/InsightsReader.swift`.
- **C-24 `InsightsSnapshot.recentActivity` orphaned after Activity tab deletion (P8 review NIT-2 carry, Track-9 follow-up).** **[DEFERRED → Track-9 stub `docs/superpowers/specs/2026-05-19-track-9-substrate-enrichment.md`]** Phase 8.8 deleted `ActivityView` — the only `Leaf/` consumer of `InsightsSnapshot.recentActivity: [ActivityFeedEntry]`. `InsightsReader.refresh()` still calls `insights.recentActivity(period: today, limit: 200)` on every tick and stores the result, but nothing reads it. The fetch path through `ActivityFeedMapper` is preserved because `LeafMCP/Tools/RecentActivityTool` still calls `recentActivity` directly via its own reader — so the substrate cannot be dropped, only the `InsightsSnapshot` field + refresh-tick fetch. Resolution = drop `InsightsSnapshot.recentActivity` + remove the `refresh()` call once Track-9 Analytics surface design is settled (Analytics may reuse the fetch or supersede with `weeklyMetrics`). Until then, ~200 rows are fetched + serialized per refresh tick for zero UI consumers. Files: `Leaf/Models/InsightsReader.swift`, `Packages/LeafCore/Sources/LeafCore/Insights/InsightsSnapshot.swift`.

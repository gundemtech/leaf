...
- **C-23 Analytics surface real content (P8 carry, Track-9 follow-up).** **[DEFERRED → Track-9 stub `docs/superpowers/specs/2026-05-19-track-9-substrate-enrichment.md`]** Phase 8.8 ships a static placeholder ("Analytics view coming soon") with no chip strip, no week breakdown, no streaks, no peak-hour, no top-tools, no WoW deltas. §5 line 273 calls for a week-scoped surface chip strip mirroring TODAY pills, and §5 line 275 explicitly defers full content design ("Track-9, to be decided after Track-8 lands"). Resolution = dedicated Track-9 brainstorm + spec covering substrate additions (`DerivedInsights.weeklyMetrics(now:)` or equivalent), LeafCorePrivate Prod impl, and the full Analytics surface design (charts, streaks, deltas). Files: `Leaf/Views/Window/Analytics/AnalyticsView.swift` + `Packages/LeafCore/Sources/LeafCore/Insights/DerivedInsights.swift`.

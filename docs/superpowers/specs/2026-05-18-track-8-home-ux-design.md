...
- **C-17 `InboxBlock.filteredItems` substring-match unit test missing.** P6 spec §8.2 deferred view-state tests, but the `filteredItems` computed property has non-trivial semantics (case-insensitivity via `lowercased()`, whitespace trimming via `trimmingCharacters(.whitespacesAndNewlines)`, multi-field OR-match on `title || sourceMeta`, empty-query short-circuit). A 5-line XCTest against synthetic `[InboxItem]` fixtures would lock the spec §3.2 contract without invoking SwiftUI. P9 polish (or split as new `InboxBlockTests.swift`). File: `Leaf/Views/Window/Home/Blocks/InboxBlock.swift:filteredItems`.
- **C-18 Empty / no-match states share the same icon.** Both `emptyDataState` ("All clear.") and `noMatchState` ("No matches.") render `LeafIcons.brand.leaf`. Semantically different — one is "inbox is genuinely empty" (positive), the other is "filter/search hides things" (neutral, user-actionable). Visually identical icon confuses scan. Resolution = either (a) introduce a filter / magnifying-glass-slash glyph for no-match, or (b) accept that the CTA difference ("nothing" vs "Clear filters") carries the semantic load. P9 polish. File: `Leaf/Views/Window/Home/Blocks/InboxBlock.swift:emptyDataState / noMatchState`.
- **C-19 `InboxItemRow.severityWord` hardcoded English.** Returns "Urgent" / "Needs response" / "Informational" inline. Consistent with broader Track-7/8 known-limitation (Localizable.strings extraction pending — see C-X of Track-7 wrap). When localization track lands, this is a touchpoint. Track-9 / Localization track. File: `Leaf/Views/Window/Home/Blocks/InboxItemRow.swift:severityWord`.

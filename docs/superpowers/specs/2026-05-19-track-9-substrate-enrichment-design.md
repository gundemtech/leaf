...
| C-25 | WhereStoppedDeriver sleep/wake idle gap | **[RESOLVED Track-10 Phase C 2026-05-23 GUN-C — Direction 1 selected; moat `ProdWhereStoppedDeriver.derive()` latest-event predicate refined to exclude system-edge context events (sleep/wake/lock/unlock). Pattern + real predicate live in the moat. 3 sentinel-injection regression tests (closed-laptop scenario, screen-lock scenario, happy-path preservation) all green. Substrate-purity preserved.]** |
...
**RESOLVED Track-10 Phase C 2026-05-23 GUN-C** — Direction 1 selected. Moat `ProdWhereStoppedDeriver.derive()` latest-event predicate refined to exclude system-edge context events (sleep/wake/lock/unlock transitions); real predicate + exact event_kind list live in the moat. Meeting / focus / attention / content / action / aiCollaboration events stay in the activity set. Idle gate now reflects real user-activity elapsed-time. 3 sentinel-injection regression tests added: `test_derive_closedLaptopScenario_emitsSnapshot` · `test_derive_screenLockedScenario_emitsSnapshot` · `test_derive_awakeIdleScenario_stillEmitsSnapshot`. Substrate-purity preserved (no new event_kinds / migrations / MCP tools). Manual verification deferred to the next real closed-laptop cycle.

...
| C-25 | WhereStoppedDeriver sleep/wake idle gap | **[RESOLVED Track-10 Phase C 2026-05-23 GUN-C — Direction 1 selected; moat `ProdWhereStoppedDeriver.derive()` `MAX(ts)` SQL augmented with `NOT (signal_type = 'context' AND event_kind IN ('system_slept', 'system_woke', 'system_locked', 'system_unlocked'))` filter. 3 sentinel-injection regression tests (closed-laptop scenario, screen-lock scenario, happy-path preservation) all green. Substrate-purity preserved.]** |
...
**RESOLVED Track-10 Phase C 2026-05-23 GUN-C** — Direction 1 selected. Moat `ProdWhereStoppedDeriver.derive()` `MAX(ts)` SQL augmented with `NOT (signal_type = 'context' AND event_kind IN ('system_slept', 'system_woke', 'system_locked', 'system_unlocked'))` filter. The 4 system-edge context event_kinds are excluded from the latest-event read; meeting / focus / attention / content / action / aiCollaboration events stay in the activity set. Idle gate now reflects real user-activity elapsed-time. 3 sentinel-injection regression tests added: `test_derive_closedLaptopScenario_emitsSnapshot` · `test_derive_screenLockedScenario_emitsSnapshot` · `test_derive_awakeIdleScenario_stillEmitsSnapshot`. Substrate-purity preserved (no new event_kinds / migrations / MCP tools). Manual verification deferred to Дима's next real closed-laptop cycle.

...
| Local conversation history file watch | **Forbidden by whitepaper won't-list (`privacy-security/what-we-dont-capture.md`).** Would yield L6 content (chat bodies). Also vendor-managed implementation detail, encrypted post-2024 patch, breaks across updates. |

**Conclusion:** there is no outbound surface from ChatGPT Desktop that yields per-event capture comparable to Claude Code hooks. Pursuing the per-event ceiling would require either (a) Apple Intelligence framework readback, which Apple does not expose; (b) reverse-engineering vendor-internal state, which whitepaper won't-list (`privacy-security/what-we-dont-capture.md`) and `/pre-push-leaf` won't-list forbid; or (c) waiting for OpenAI to ship an outbound surface, which they have not.
...
- «Why don't you just FSEvents-watch the conversation dir for activity-pulse metadata?» — Answer: vendor-internal store, no stability contract, whitepaper won't-list (`privacy-security/what-we-dont-capture.md`) discipline > thin signal duplicating L1/L2.
...
### **AI co-pilot surfaces без per-event API**
...
| **GitHub Copilot** | (IDE-embedded — no standalone bundle) | L1 attention proxied through host IDE (Xcode / VS Code / JetBrains) — indistinguishable from human typing within that IDE | Per-suggestion / per-acceptance telemetry | Copilot exposes only org-aggregate via REST `/copilot/usage`. No per-event outbound surface. IDE-side extension events exist but live under Layer D V2 (separate track per contract §11). |
...
10. Pre-push diff scan against `gundemtech/leaf` (this repo) per `/pre-push-leaf` checklist. **Responsibility:** author at push time (the human, not the Claude session) — Claude self-review in §8 audits phrasing for moat leakage as a *first pass*; final `/pre-push-leaf` invocation runs interactively by author when ready to push. P7 self-review §8 covers Claude's pass; author's pass is a separate gate at collective Track-6 integration merge time.
...
- [x] Quotes / citations for "no API" claims — all 7 entries in this spec's §2 surface-audit table cite either OpenAI help doc, OpenAI developers doc, Apple support, or research doc §2.1 cross-reference (which holds the full citation set).

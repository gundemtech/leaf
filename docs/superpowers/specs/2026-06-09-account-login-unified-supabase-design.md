...
- **Статус:** реализовано на ветке `feature/account-login-phase1` — см. «Реализация (2026-06-10)» в конце §2 (зафиксированы отступления от плана)
...
### Реализация (2026-06-10) — отступления от плана, зафиксированы

Ветка `feature/account-login-phase1`. По ходу реализации приняты осознанные расхождения с решениями ниже — план не переписывался, фиксируем здесь как источник правды по as-built:

1. **CAPTCHA — выключена, не включена.** Turnstile-виджет в приле НЕ делался; нативный логин шлёт пустой `captchaToken` (ключ `gotrue_meta_security` опускается). Предусловие: на дашборде проекта global CAPTCHA должен быть **OFF**, иначе password-логин отлетит. (Отменяет §5.A/§9.2 о CAPTCHA-ON + WKWebView-виджете.)
2. **OAuth-редирект — loopback + системный браузер, не ASWebAuthenticationSession.** Используется `http://127.0.0.1:47825/callback` (тот же паттерн, что Linear/Slack/GitHub), открывается дефолтный браузер юзера. **Дашборд:** этот loopback-URL обязан быть в Redirect URLs проекта (не `leaf://auth/callback`). (Отменяет §5.C/§9.3.)
3. **CSRF — через PKCE, без своего `state`.** Защита OAuth — PKCE (код привязан к `code_verifier`, не покидает процесс), как в supabase-js. Свой `state` в `/authorize` НЕ шлём: GoTrue брокерит поток и владеет провайдерным state сам — caller-`state` ломает его с `bad_oauth_state` (поймано на живом смоуке 2026-06-11; финдинг #7 «нет CSRF state» для брокер-потока ложный — PKCE его закрывает).
4. **Sign-out — реализован** (а не «future task»): Settings → Account → кнопка «Sign Out» (с confirm) → `SupabaseClient.signOut()` (чистит in-memory + персист-файл) + `launchAgent.unregister()` + гейт возвращается на `LoginGateView`.
5. **Offline-grace — на уровне гейта** (уточняет §5.D: транзиентная ошибка ≠ `exit(0)`/логин). Cold-start гейт (RootView + Agent) различает транзиентную ошибку (`SupabaseError.isTransientNetworkFailure`: `.transport`/`.serverError`/`.rateLimited`) от auth-reject (`.unauthorized`): при оффлайне с персист-сессией юзер остаётся в shell, агент продолжает локальный захват (не `exit(0)`). Внутри-сессионного ре-пробинга нет — revoked-while-offline держит shell до рестарта (осознанный трейд-офф §5.D).
6. **Гейт проверяет только сессию, не pubkey.** Cold-start гейт зовёт `ensureAuthenticated()`, а не `ensureAuthenticatedAndPubkeyRegistered()` — осознанно, ради offline-resilience и без лишнего сетевого раунда. Pubkey регистрируется на логине (`finishWithDeviceRegistration`), 409-коллизия теперь обрабатывается идемпотентно. (Лёгкое отступление от §4 «сессия валидна И pubkey зарегистрирован».)

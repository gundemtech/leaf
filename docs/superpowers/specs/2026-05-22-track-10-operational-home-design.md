...
| C-T10-EMIT-T9-A11Y | 5 a11y NITs deferred from T9 a11y sweep (subagent verdict 2026-05-23): TeamNBlock "Team, N active" 0/1 pluralization edge; YoureOnBlock truncated taskLine VO read (verify composer a11yLabel covers); SinceLastActiveBlock "Mark all as seen" missing `accessibilityHint`; RecapBlock/EodBlock empty-state VO refresh on header toggle (`.combine` on outer VStack); ResumeHeroBlock literal quote-marks audible in subject/WIP `.accessibilityLabel`. | T9 a11y sweep 2026-05-23 | OUT — post-Track-10 own a11y polish phase. Pair with T7 5 a11y NITs forward (originally tagged "T10" in T7 a11y spec before Track-10 finalized to 9 phases). |
| C-T10-EMIT-T9-HIG | 5 HIG NITs deferred from T9 HIG sweep (subagent verdict 2026-05-23): TeamNBlock avatar raw `32×32` literal → `LeafSize.avatarMd` design token; StandupHeaderRow chevron raw `frame(width: 12)` → `LeafSize.chevronWidth`; YoureOnBlock empty-state copy mixes title+explainer (split into `LeafEmptyState(title:description:)` pattern); RecapBlock/EodBlock plain `Text` empty-state → `LeafEmptyState` pattern (collapsible-card footprint caveat); ResumeHeroBlock manual `prefix(60)` truncation → native `.lineLimit/.truncationMode`. | T9 HIG sweep 2026-05-23 | OUT — post-Track-10 design-system polish phase. Coordinate with C-T10-EMIT-T9-A11Y (`LeafEmptyState` adoption gives a11y `.combine` for free). |
| C-T10-EMIT-T9-A11Y-PRIMITIVES | Design-system primitive a11y audit forward carry from Track-9 T10 — `LeafPill` / `LeafInput` / `LeafIconChip` as Button labels need consistent traits (`.isButton`, optional `.isSelected`). Currently each callsite manually adds traits (NeedsYouFilterRow / SinceFilterRow / TeamNBlock). Hoist into primitive view modifier. | Track-9 T10 carry, re-affirmed T9 | OUT — post-Track-10 design-system primitive audit phase. Reduces a11y-trait drift across future blocks. |
| C-T10-EMIT-T7-A11Y | 5 a11y NITs from T7 Stage 6 a11y reviewer (verdict 2026-05-23) originally tagged "T10" — section-label `.isHeader` discipline pre-T9 a11y sweep; WeekChipStrip today selection `.isSelected`; Timer-driven refresh accessibility announcements; copy polish; design-system primitive a11y audit. T9 a11y sweep closed 4 of these inline (.isHeader on RESUME/NEEDS YOU/SINCE/TODAY) — 1 truly remaining + WeekChipStrip Analytics-hidden item. | T7 a11y reviewer 2026-05-23 | OUT — post-Track-10 own a11y phase (D-2 forward carry — T7 a11y reviewer wrote spec before Track-10 finalized to 9 phases; "T10" semantic = forward carry). |

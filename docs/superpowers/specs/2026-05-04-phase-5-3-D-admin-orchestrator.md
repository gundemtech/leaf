...
| R5 | `postedAtMs` IS NULL filter on partial index not respected by some SQLite versions | Mitigated by SQLite version (partial indexes supported since 3.8.0). Test #12 verifies partial index actually filters. |

...
enum AgentMain {
    static func main() {
        ...
        // Track-6 P4 — start Google Calendar collector. Skipped gracefully when
        // OAuth client_id/secret are not configured at build time.
...
enum AgentLifetime {
    ...
    // Track-6 P3 — browser bookmark FSEvents watcher.
    nonisolated(unsafe) static var browserBookmarksWatcher: BrowserBookmarksWatcher?
    // Track-6 P3 — darwin notification observer token for browser allow-list
    // cross-process invalidation. Retained for Agent lifetime.
